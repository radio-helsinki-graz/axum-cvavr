...
#include "Axum-Rack-MICAD.h"
#include "CANTransportLayer.h"
#include "Axum-Rack-MICAD-MambaNet.h"
...
void main(void)
{
   ...
   // State7=T State6=T State5=0 State4=1 State3=0 State2=0 State1=1 State0=0
   PORTD=0x12;
   DDRD=0xBF;
   ...
   // State4=T State3=0 State2=1 State1=0 State0=0
   PORTG=0x04;
   DDRG=0x0F;
...
void DoSwitch(unsigned char LogicSwitchNr, int Event)
{
   if ((Event == 1) || (Event == -1))
   {
      ...
      else if ((LogicSwitchNr>=8) && (LogicSwitchNr<16))
      {
         ...
         if ((LogicSwitchNr>=8) && (LogicSwitchNr<12))
         {
            MICClip[LogicSwitchNr-8] = TransmitBuffer[0];
         }
         else if ((LogicSwitchNr>=12) && (LogicSwitchNr<16))
         {
            ADOverload[LogicSwitchNr-12] = TransmitBuffer[0];
         }
      }
      ...
      {  //Object message
         ...
         switch (ObjectMessageAction)
         {
            ...
            case  MAMBANET_OBJECT_ACTION_GET_SENSOR_DATA:
            {
               ...
               else if ((ObjectNr>=1091) && (ObjectNr<1095))
               {  //MIC=Preamp-clip
                  TransmitBuffer[0] = (ObjectNr>>8)&0xFF;
                  TransmitBuffer[1] = ObjectNr&0xFF;
                  TransmitBuffer[2] = MAMBANET_OBJECT_ACTION_SENSOR_DATA_RESPONSE;
                  TransmitBuffer[3] = STATE_DATATYPE;
                  TransmitBuffer[4] = 1;
                  TransmitBuffer[5] = MICClip[ObjectNr-1091];

                  SendMambaNetMessageToCAN(FromAddress, LocalMambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 6);

                  MessageDone = 1;
               }
               else if ((ObjectNr>=1095) && (ObjectNr<1099))
               {  //Overload ADC
                  TransmitBuffer[0] = (ObjectNr>>8)&0xFF;
                  TransmitBuffer[1] = ObjectNr&0xFF;
                  TransmitBuffer[2] = MAMBANET_OBJECT_ACTION_SENSOR_DATA_RESPONSE;
                  TransmitBuffer[3] = STATE_DATATYPE;
                  TransmitBuffer[4] = 1;
                  TransmitBuffer[5] = ADOverload[ObjectNr-1095];

                  SendMambaNetMessageToCAN(FromAddress, LocalMambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 6);

                  MessageDone = 1;
               }
               else if ((ObjectNr>=1099) && (ObjectNr<1103))
               {  //Mic signal>-28dB
            ...
            case  MAMBANET_OBJECT_ACTION_GET_ACTUATOR_DATA:
            {
               ...
               else if ((ObjectNr>=1103) && (ObjectNr<1107))
               {  //MIC Phantom
                  unsigned char ChannelNr;
                  ChannelNr = ObjectNr-1103;

                  TransmitBuffer[3] = STATE_DATATYPE;
                  TransmitBuffer[4] = 1;
                  TransmitBuffer[5] = MICPhantom[ChannelNr];

                  SendMambaNetMessageToCAN(FromAddress, LocalMambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 6);

                  MessageDone = 1;
               }
               else if ((ObjectNr>=1107) && (ObjectNr<1111))
               {  //MIC Pad
                  unsigned char ChannelNr;
                  ...
                  TransmitBuffer[3] = STATE_DATATYPE;
                  TransmitBuffer[4] = 1;
                  TransmitBuffer[5] = MICPad[ChannelNr];

                  SendMambaNetMessageToCAN(FromAddress, LocalMambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 6);

                  MessageDone = 1;
               }
               else if ((ObjectNr>=1111) && (ObjectNr<1115))
               {  //MIC Gain
                  unsigned char ChannelNr;
                  unsigned char VariableFloat[2];

                  ChannelNr = ObjectNr-1111;
                  if (Float2VariableFloat(MICGain[ChannelNr], 2, VariableFloat) == 0)
                  {
               ...
               else if ((ObjectNr>=1115) && (ObjectNr<1119))
               {  //Input routing
                  unsigned char ChannelNr;
               ...
               else if ((ObjectNr>=1119) && (ObjectNr<1123))
               {  //Input phase
                  unsigned char ChannelNr;
                  ChannelNr = ObjectNr-1119;
               ...
               {  //MIC Gain
                  ...
                  if (DataType == FLOAT_DATATYPE)
                  {
                     if (DataSize == 2)
                     {
                        ...
                        {
                           MICGain[ChannelNr] = FloatData;
                           if (MICGain[ChannelNr]>75)
                           {
                              MICGain[ChannelNr] = 75;
                           }
                           else if (MICGain[ChannelNr]<20)
                           {
                              MICGain[ChannelNr] = 20;
                           }
                           SetInputRoutingAndLevel(ChannelNr);
...
void SetInputRoutingAndLevel(unsigned char ChannelNr)
{
   ...
   PGA2500Data[(unsigned char)(PGANr<<1)] = 0x50;
   if (MICPhantom[ChannelNr])
   {
      PGA2500Data[(unsigned char)(PGANr<<1)] |= 0x02;
   }
   if (MICPad[ChannelNr])
   {
      PGA2500Data[(unsigned char)(PGANr<<1)] |= 0x01;
   }
   IntegerLevel = MICGain[ChannelNr]-19;
   if (IntegerLevel<1)
   {
      IntegerLevel = 1;
   }
   ...
   PGA2500Data[(unsigned char)((PGANr<<1)+1)] = IntegerLevel;
   SetPGA2500s();
...
void SetPGA2500s()
{
   ...
   for (cntByte=0; cntByte<8; cntByte++)
   {
      ...
      {
         Mask = 0x80>>cntBit;
         SDI_PGA1 = PGA2500Data[cntByte]&Mask;
         SCLK_PGA = 1;
         SCLK_PGA = 0;
      }
   }
