...
void main(void)
{
   ...
   while (1)
   {
      ...
      if (cntMilliSecond - PreviousMilliSecondReservation > 1000)
      {
         ...
         {
            SendMambaNetReservationRequest();
         }
      }
         ...
         switch (ObjectMessageAction)
         {
            ...
            case  MAMBANET_OBJECT_ACTION_GET_SENSOR_DATA:
            {
               ...
               {  //Rack slot address
                  ...
                  SendMambaNetMessageToCAN(FromAddress, LocalMambaNetAddress, MessageID, 1, TransmitBuffer, 6);

                  MessageDone = 1;
               }
               else if ((ObjectNr>=1025) && (ObjectNr<1027))
               {  //FPGA Firmware version
               }
               else if ((ObjectNr>=1035) && (ObjectNr<1037))
               {  //ADAT-input-status
               }
               else if ((ObjectNr>=1037) && (ObjectNr<1053))
            ...
            case  MAMBANET_OBJECT_ACTION_SET_ACTUATOR_DATA:
            {
               ...
               if ((ObjectNr>=1027) && (ObjectNr<1031))
               {  //LED
               ...
               else if ((ObjectNr>=1031) && (ObjectNr<1035))
               {  //LED mode
                  if (DataType == STATE_DATATYPE)
                  {
                     if (DataSize == 1)
